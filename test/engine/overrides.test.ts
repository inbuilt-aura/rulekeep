...
describe('findOverride', () => {
  it('matches an override on the same line', () => {
    const lines = ['const x = y as any; // rulekeep-ignore no-any: legacy API, tracked in #42'];
  ...
  it('matches an override on the line directly above', () => {
    const lines = ['// rulekeep-ignore no-any: third-party JSON, validated below', 'const x = y as any;'];
  ...
  it('requires a reason: a bare ignore comment does not count', () => {
    const lines = ['const x = y as any; // rulekeep-ignore no-any'];
    expect(findOverride('no-any', lines, 1)).toBeUndefined();
  });

  it('requires the rule id to match exactly', () => {
    const lines = ['const x = y as any; // rulekeep-ignore no-console-log: reason here'];
    expect(findOverride('no-any', lines, 1)).toBeUndefined();
  });

  it('does not look further than one line above', () => {
    const lines = ['// rulekeep-ignore no-any: reason', '', 'const x = y as any;'];
