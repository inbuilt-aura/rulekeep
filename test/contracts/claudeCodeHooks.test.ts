...
import {
  ...
  handleSessionStart,
  handleStop,
} from "../../src/cli/hookClaudeCode.js";
...
describe("PreToolUse — commands", () => {
  it("denies a blocked command with the exact permissionDecision shape", () => {
    const output = handlePreToolUse(
      bashPayload("git push origin main --force"),
    ) as {
      hookSpecificOutput?: {
...
describe("PostToolUse — edits", () => {
  ...
  it("stays silent on a clean edit", () => {
    expect(
      performEdit("src/clean.ts", "export const a = 1;\n", "tool-3"),
    ).toEqual({});
  });
...
describe("Stop", () => {
  ...
  it("sends the agent back when a blocking rule is still open", () => {
    ...
    const output = handleStop(stopPayload(false)) as {
      decision?: string;
      reason?: string;
    };
    expect(output.decision).toBe("block");
  ...
  it("gives up after maxStopRetries instead of looping forever", () => {
    ...
    expect(
      (handleStop(stopPayload(false)) as { decision?: string }).decision,
    ).toBe("block");
    expect(
      (handleStop(stopPayload(true)) as { decision?: string }).decision,
    ).toBe("block");

    const third = handleStop(stopPayload(true)) as {
      decision?: string;
  ...
  it("does not re-block the same finding on the next turn after giving up", () => {
    performEdit("src/bad.ts", "const data = x as any;\n", "tool-c");
    handleStop(stopPayload(false));
    handleStop(stopPayload(true));
    handleStop(stopPayload(true));

    expect(handleStop(stopPayload(false))).toEqual({});
  });

  it("does not grade a recorded file after the shell deletes it", () => {
    performEdit("src/deleted.ts", "const data = x as any;\n", "tool-deleted");
    rmSync(join(repo, "src/deleted.ts"));

    expect(handleStop(stopPayload(false))).toEqual({});
  });

  it("catches a shell-only edit at stop", () => {
    ...
    expect((output as { decision?: string; reason?: string }).decision).toBe(
      "block",
    );
    expect((output as { reason?: string }).reason).toContain("src/shell.ts:1");
  });
  ...
  it("reports overrides used, so a silenced rule is never invisible", () => {
    performEdit(
      "src/ov.ts",
      "const d = x as any; // rulekeep-ignore no-any: vendor types\n",
      "tool-e",
    );
