...
describe("rulekeep check", () => {
  ...
  it("grades an untracked new file without emitting git fatal noise", () => {
    write("src/new.ts", "const data = y as any;\n");

    const result = check();
    expect(result.exitCode).toBe(1);
    expect(result.output).toContain("src/new.ts:1");
  });
  ...
  it("reports overrides rather than hiding them", () => {
    write(
      "src/ov.ts",
      "const d = y as any; // rulekeep-ignore no-any: vendor types are wrong\n",
    );
