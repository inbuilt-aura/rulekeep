/**
 * `rulekeep trust` — approve, list or revoke this repo's checker commands
 * (docs/03-architecture.md "Checker commands need approval"). The
 * /rulekeep:trust skill calls this; a user can also run it directly.
 *
 * Approving is deliberately a separate, explicit act: it is the one place
 * rulekeep agrees to run a command that came out of a repo's config file.
 */
  ...
  switch (action) {
    ...
    case 'revoke': {
      ...
      return {
        exitCode: 0,
        output: removed
          ? `rulekeep: revoked approval for ${loaded.path}. Its checkers will not run until approved again.`
          : `rulekeep: nothing to revoke — ${loaded.path} was not approved.`,
      };
    }
    case 'approve': {
