...
var isActive = (finding) => finding.override === void 0;
...
var RULE_TYPES = ["command", "line", "boundary", "test-guard", "checker", "prose"];
var TEST_GUARD_CHECKS = ["skip-or-focus", "file-deleted", "assertions-removed"];
var DEFAULT_MODE = "warn";
var DEFAULT_MAX_STOP_RETRIES = 3;
var DEFAULT_TIMEOUT_SECONDS = 60;
var MAX_TIMEOUT_SECONDS = 3600;
var MODES = ["off", "warn", "block"];
function lineOf(doc, node) {
  const range = node?.range;
  if (!range || range[0] === void 0) return 1;
...
function compileRegex(source, errors, line, field) {
  ...
  try {
    ...
    errors.push({ line, message: `"${field}" is not a valid regular expression: ${cause.message}` });
    return void 0;
  }
}
function parseConfig(source) {
  const doc = (0, import_yaml.parseDocument)(source, { strict: true });
  ...
  const defaults = rootJson.defaults ?? {};
  const defaultMode = MODES.includes(defaults.mode) ? defaults.mode : DEFAULT_MODE;
  const defaultAllowOverride = defaults.allowOverride !== false;
  const maxStopRetries = typeof defaults.maxStopRetries === "number" && Number.isInteger(defaults.maxStopRetries) && defaults.maxStopRetries >= 0 ? defaults.maxStopRetries : DEFAULT_MAX_STOP_RETRIES;
  ...
  rulesJson.forEach((raw, index) => {
    ...
    const rule = raw;
    const id = typeof rule.id === "string" && rule.id.trim().length > 0 ? rule.id : void 0;
    if (!id) {
    ...
    switch (type) {
      ...
      case "line": {
        const added = rule.added === void 0 ? void 0 : compileRegex(rule.added, errors, line, "added");
        const removed = rule.removed === void 0 ? void 0 : compileRegex(rule.removed, errors, line, "removed");
        if (rule.added === void 0 && rule.removed === void 0) {
          errors.push({ line, message: `Rule "${id}": a "line" rule needs "added" and/or "removed".` });
          return;
        }
        if (rule.added !== void 0 && !added || rule.removed !== void 0 && !removed) return;
        rules.push({ ...common, type: "line", added, removed });
        break;
      }
      case "boundary": {
        const disallow = Array.isArray(rule.disallow) ? rule.disallow.filter((v) => typeof v === "string") : [];
      ...
      case "test-guard": {
        const checks = Array.isArray(rule.checks) ? rule.checks.filter((c) => TEST_GUARD_CHECKS.includes(c)) : [...TEST_GUARD_CHECKS];
        rules.push({
          ...common,
          mode: typeof rule.mode === "string" && MODES.includes(rule.mode) ? rule.mode : "block",
      ...
      case "checker": {
        const run = typeof rule.run === "string" && rule.run.trim().length > 0 ? rule.run : void 0;
        if (!run) {
        ...
        if (rawTimeout !== void 0 && (typeof rawTimeout !== "number" || !Number.isFinite(rawTimeout) || rawTimeout <= 0 || rawTimeout > MAX_TIMEOUT_SECONDS)) {
        ...
        const when = Array.isArray(rule.when) && rule.when.length > 0 ? (0, import_picomatch.default)(rule.when) : void 0;
...
function ruleAppliesTo(rule, path) {
  return rule.matchesPath === void 0 || rule.matchesPath(path);
}
...
// src/engine/diff.ts
var normalize = (text) => text.replace(/\r\n/g, "\n");
...
function parseOverride(line) {
  ...
  if (!ruleId || !reason) return void 0;
  return { ruleId, reason };
}
function findOverride(ruleId, lines, lineNumber) {
...
// src/engine/rules/command.ts
function checkCommandRule(rule, event) {
...
// src/engine/rules/line.ts
var MAX_LINE_LENGTH = 2e3;
function findingsFor(rule, path, pattern, hits, afterText) {
  if (!pattern) return [];
  ...
  for (const { line, text } of hits) {
    ...
    const override = afterLines.length > 0 ? findOverride(rule.id, afterLines, line) : void 0;
    findings.push(override ? { ...base, override } : base);
  }
  return findings;
}
function checkLineRule(rule, event) {
  if (event.kind !== "after-edit" && event.kind !== "stop" || rule.mode === "off") return [];
  const findings = [];
  for (const change of event.changes) {
    if (!ruleAppliesTo(rule, change.path)) continue;
...
// src/engine/rules/prose.ts
function checkProseRule(rule, event) {
...
// src/engine/rules/testGuard.ts
var SKIP_OR_FOCUS = /\.(skip|only|todo)\(|^\s*xit\s*\(|^\s*xdescribe\s*\(|skip:\s*true|@pytest\.mark\.skip|\bt\.Skip\(/;
var ASSERTION = /\bexpect\(|\bassert[.(]|\bshould\b/;
function findSkipOrFocus(rule, path, before, after) {
...
function findDeletedFile(rule, path, before, after) {
...
function findRemovedAssertions(rule, path, before, after) {
  if (before === null || after === null) return [];
  ...
  return [
    {
      ruleId: rule.id,
      mode: rule.mode,
      message: rule.message || `${removedAssertions - addedAssertions} more assertion(s) were removed than added. Fix the code under test, not the test.`,
      path
    }
  ];
}
function checkTestGuardRule(rule, event) {
  if (event.kind !== "after-edit" && event.kind !== "stop" || rule.mode === "off") return [];
  const findings = [];
  for (const change of event.changes) {
    if (!ruleAppliesTo(rule, change.path)) continue;
...
function evaluate(rules, event, extra = []) {
...
function locationOf(finding) {
  if (finding.path === void 0) return "";
  return finding.line === void 0 ? `  ${finding.path}` : `  ${finding.path}:${finding.line}`;
}
function lineFor(finding) {
...
function trimOutput(text, maxLines = MAX_OUTPUT_LINES) {
  const lines = stripAnsi(text).replace(/\r\n/g, "\n").split("\n");
  while (lines.length > 0 && lines[lines.length - 1]?.trim() === "") lines.pop();
  const kept = lines.slice(0, maxLines).map((line) => line.length > MAX_LINE_LENGTH2 ? `${line.slice(0, MAX_LINE_LENGTH2)}\u2026` : line);
  const dropped = lines.length - kept.length;
  if (dropped > 0) kept.push(`\u2026 ${dropped} more line(s)`);
  return kept.join("\n");
}
function runChecker(rule, repoRoot) {
  const cwd = (0, import_node_path.isAbsolute)(rule.cwd) ? rule.cwd : (0, import_node_path.resolve)(repoRoot, rule.cwd);
  const timeout = Math.min(Math.max(Math.floor(rule.timeoutSeconds * 1e3), 1), MAX_TIMEOUT_MS);
  let result;
  try {
    result = (0, import_node_child_process.spawnSync)(rule.run, {
      cwd,
      shell: true,
      encoding: "utf8",
      timeout,
    ...
    return { ok: false, timedOut: false, output: `could not run: ${cause.message}` };
  }
...
function checkerApplies(rule, event) {
  if (rule.mode === "off") return false;
  const phase = event.kind === "stop" ? "stop" : event.kind === "after-edit" ? "edit" : void 0;
  if (phase === void 0 || phase !== rule.on) return false;
...
function runCheckers(rules, event, repoRoot) {
  const findings = [];
...
function findConfigPath(startDir) {
  ...
  for (let i = 0; i < MAX_ANCESTORS; i += 1) {
    const candidate = (0, import_node_path2.join)(dir, CONFIG_FILENAME);
...
function loadConfig(startDir) {
  ...
  if (!path) return { ok: false, errors: [], path: void 0 };
...
function fingerprint(rules) {
  const material = rules.map((rule) => JSON.stringify([rule.id, rule.run, rule.cwd, rule.on, rule.timeoutSeconds])).sort().join("\n");
  return (0, import_node_crypto.createHash)("sha256").update(material).digest("hex");
}
function readTrustFile() {
  const path = trustFilePath();
  if (!(0, import_node_fs3.existsSync)(path)) return { version: 1, entries: {} };
  try {
...
function keyFor(configPath) {
  return (0, import_node_path4.resolve)(configPath).replace(/\\/g, "/");
}
function isTrusted(configPath, rules) {
  if (rules.length === 0) return true;
  const entry = readTrustFile().entries[keyFor(configPath)];
  return entry !== void 0 && entry.fingerprint === fingerprint(rules);
}
function trust(configPath, rules) {
  const path = trustFilePath();
  ...
  const next = {
    ...
    entries: {
      ...current.entries,
      [keyFor(configPath)]: { fingerprint: fingerprint(rules), approvedAt: (/* @__PURE__ */ new Date()).toISOString() }
    }
  };
  (0, import_node_fs3.writeFileSync)(path, `${JSON.stringify(next, null, 2)}
`, "utf8");
}
function revoke(configPath) {
  const path = trustFilePath();
  const current = readTrustFile();
  const entries = { ...current.entries };
  if (entries[keyFor(configPath)] === void 0) return false;
  delete entries[keyFor(configPath)];
  (0, import_node_fs3.mkdirSync)((0, import_node_path4.dirname)(path), { recursive: true });
  (0, import_node_fs3.writeFileSync)(path, `${JSON.stringify({ version: 1, entries }, null, 2)}
`, "utf8");
  return true;
}
function untrustedNotice(configPath, rules) {
  if (rules.length === 0 || isTrusted(configPath, rules)) return void 0;
...
// src/cli/check.ts
function githubAnnotation(level, path, line, message) {
...
${message}` };
  ...
  const event = {
    kind: "stop",
    // reuses the stop-time rule set: line, boundary, test-guard (docs/03-architecture.md "Which rules run when")
    agent: "ci",
    sessionId: "ci",
  ...
  const active = verdict.findings.filter((f) => f.override === void 0);
  const overridden = verdict.findings.filter((f) => f.override !== void 0);
  if (options.format === "json") {
    return { exitCode: verdict.outcome === "block" ? 1 : 0, output: JSON.stringify(verdict, null, 2) };
  }
  if (options.format === "github") {
    const lines = verdict.findings.map(
      (f) => githubAnnotation(f.mode === "block" && f.override === void 0 ? "error" : "warning", f.path, f.line, `${f.ruleId}: ${f.message}`)
    );
...
function toClaudeBeforeCommandOutput(verdict, message) {
...
function toClaudeAfterEditOutput(verdict, message) {
  if (verdict.outcome === "block") {
...
function toClaudeStopOutput(verdict, reason, exhaustedSummary) {
  if (verdict.outcome === "block" && exhaustedSummary === void 0) {
    return { decision: "block", reason };
  }
  return exhaustedSummary ? { systemMessage: exhaustedSummary } : {};
}
function toClaudeSessionStartOutput(reminder) {
  return reminder ? { hookSpecificOutput: { hookEventName: "SessionStart", additionalContext: reminder } } : {};
}

// src/adapters/paths.ts
var import_node_path5 = require("node:path");
function toRepoRelative(absolutePath, repoRoot) {
  return (0, import_node_path5.relative)(repoRoot, absolutePath).replace(/\\/g, "/");
}
...
function sessionDir(agent, sessionId) {
  return (0, import_node_path6.join)((0, import_node_os2.tmpdir)(), ROOT_DIR_NAME, `${safe(agent)}-${safe(sessionId)}`);
}
...
function readChanges(agent, sessionId) {
  ...
  const firstBefore = /* @__PURE__ */ new Map();
  const lastAfter = /* @__PURE__ */ new Map();
  const order = [];
...
function writeStopRetries(agent, sessionId, count) {
  const dir = sessionDir(agent, sessionId);
  ensureDir(dir);
  (0, import_node_fs4.writeFileSync)((0, import_node_path6.join)(dir, RETRY_FILE), JSON.stringify({ count }), "utf8");
}
function cleanupStaleSessions() {
  const root = (0, import_node_path6.join)((0, import_node_os2.tmpdir)(), ROOT_DIR_NAME);
  if (!(0, import_node_fs4.existsSync)(root)) return;
  const now = Date.now();
  for (const entry of (0, import_node_fs4.readdirSync)(root)) {
    const dir = (0, import_node_path6.join)(root, entry);
    try {
...
function saveSnapshot(agent, sessionId, toolUseId, contentOrNull) {
  const path = snapshotPath(agent, sessionId, toolUseId);
  (0, import_node_fs5.mkdirSync)((0, import_node_path7.join)(path, ".."), { recursive: true });
  (0, import_node_fs5.writeFileSync)(path, contentOrNull === null ? MISSING_MARKER : contentOrNull, "utf8");
}
function takeSnapshot(agent, sessionId, toolUseId) {
  const path = snapshotPath(agent, sessionId, toolUseId);
  if (!(0, import_node_fs5.existsSync)(path)) return { found: false };
  const raw = (0, import_node_fs5.readFileSync)(path, "utf8");
  (0, import_node_fs5.rmSync)(path, { force: true });
  return { found: true, content: raw === MISSING_MARKER ? null : raw };
}
...
// src/cli/trust.ts
function runTrust(repoRoot, action) {
  const loaded = loadConfig(repoRoot);
  if (!loaded.ok) {
    if (loaded.path === void 0) {
...
// src/cli/main.ts
async function runHook(agent, event) {
...
function runCheckCommand(args) {
  ...
  const base = baseIndex >= 0 ? args[baseIndex + 1] : void 0;
...
function runTrustCommand(args) {
  const action = args.includes("--revoke") ? "revoke" : args.includes("--list") ? "list" : "approve";
