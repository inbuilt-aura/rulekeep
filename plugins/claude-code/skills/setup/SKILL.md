...
3. Propose rulekeep.yaml using only these rule types:
...
6. Only write rulekeep.yaml after the user approves. Then run
   `rulekeep doctor` and report the result.

If the file proposes any `checker` rules, tell the user they must run
`/rulekeep:trust` before those commands will run.
